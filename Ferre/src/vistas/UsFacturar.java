...
public class UsFacturar extends JFrame {
	...
	public UsFacturar() {
		...
		btnCrearNuevo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String añadir= añad.getText();
				int id=0;
				boolean n=false;
				Select buscar=new Select();
	            try{
	                id=Integer.parseInt(añadir);
	                n =true;
	            }catch(NumberFormatException e){
	                n =false;
	            }
	            if (n== true) {
					...
					ArrayList<Productos> ho=buscar.nomProductos(añadir);
					Iterator<Productos> nombreIterator = ho.iterator();
		...
		añad = new JTextField();
		añad.setColumns(10);
		añad.setBounds(336, 383, 116, 22);
		contentPane.add(añad);
